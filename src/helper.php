...
namespace Angujo\Lareloquent {
    ...
    if (!function_exists('flatten_array')) {
        function flatten_array(array $items, array $flattened = []): array
        {
    ...
    if (!function_exists('model_name')) {
        function model_name(string $txt): string
        {
            return preg_replace_callback('/([A-Z])([a-z\d]+)$/',
                function ($matches) {
                    return in_singular($matches[1] . $matches[2]);
                },
                preg_replace_callback('/(^|[^A-Za-z\d])([a-zA-Z])([A-Z]+|[a-z\d]+)/', function ($matches) {
    ...
    if (!function_exists('snake_case')) {
        function snake_case(string $txt): string
        {
            return preg_replace_callback('/([^a-z\d])([a-z\d]+)/', function ($matches) {
                return '_' . strtolower($matches[1]) . strtolower($matches[2]);
            }, $txt);
        }
    }

    if (!function_exists('col_name_reference')) {
        function col_name_reference(string $txt): string
        {
    ...
    if (!function_exists('enum_case')) {
        function enum_case(string $txt): string
        {
    ...
    if (!function_exists('number_hash')) {
        function number_hash(string $str, int $base = 10): string
        {
    ...
    if (!function_exists('method_name')) {
        function method_name(string $txt): string
        {
    ...
    if (!function_exists('str_rand')) {
        function str_rand(int $length = 5, $upper = true, $lower = true, $numbers = true, $special_xters = true): string
        {
            if ($length <= 0) return '';
    ...
    if (!function_exists('tag')) {
        function tag(string $name, string $description): GeneralTag
        {
    ...
    if (!function_exists('in_singular')) {
        function in_singular(string $word): string
        {
            $infl = new Inflector();
    ...
    if (!function_exists('model_file')) {
        function in_plural(string $word, float $count = 99): string
        {
            $infl = new Inflector();
