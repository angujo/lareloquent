...
class Resource extends FileCreator
{
    ...
    private function toArrayMethod()
    {
        $columns = array_filter($this->columns,
            fn(DBColumn $column) => !in_array($column->column_name, LarEloquent::config()->soft_delete_columns)
                && !(array_key_exists($this->table->name, LarEloquent::config()->hidden_columns) && in_array($column->column_name, LarEloquent::config()->hidden_columns[$this->table->name])));
        $body = ['$defaults = ' .
